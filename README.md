...
$$
\begin{cases}c_i \gets c_i + 1 \\ p_i \gets p_i + r_i\end{cases}
$$
...
$$
\begin{cases}parent_s \gets l \\ p_s \gets p_s - p_l \\ c'_s \gets c_l \\ r'_s \gets r_l\end{cases}
$$
...
typedef struct lazyTag{
    ...
    ll raise_salary_sum, pay_salary_sum; //r, p
    int pay_salary_cnt; // c
    ll raise_salary_sum_when_merge; //r'
...
pair get_act_salary(int p){
    ...
    return make_pair(res.first
                   + lazy[p].pay_salary_sum
                   + res.second * (lazy[p].raise_salary_sum - lazy[p].raise_salary_sum_when_merge),
                //    - (lazy[dsu[p]].pay_salary_cnt - lazy[p].pay_salary_cnt_when_merge) * lazy[p].raise_salary_sum_when_merge,
                     res.second);
}
